...
**TTS 旁白字段约束**（在「待生成素材」中必填）：
- TTS source：`local-TTS` / `MiniMax` / `human` / `none` 四选一
- voice_id：必填（如 `male-qn-qingse` / `female-shaonv` / 本地 TTS 对应 ID）
- speed：必填（本地 TTS 默认 1.0；MiniMax 推荐 1.6）
- 输出路径：必填（如 `voice/scene-XX.mp3`）
- 真人录音需额外标录音环境（房间 / 录音棚）
...
- **scene 时长反推规则**：TTS 路径必须先按文本估算生成音频，用 `ffprobe` 实测音频时长，再用 `scene_duration = max(audio_dur + 2.0, 6.0)` 反推 scene duration —— 不用 3-4 字/秒估算（MiniMax speech-2.8-hd 实测比理论慢 27%，估算必翻车）
- **浮点重叠防御**：相邻 `data-start + data-duration` 严格 `<` 下一片段 `data-start`；遇浮点累积时对 `data-duration` 向下取整 0.1s，或对后一片段 `data-start` 加 +0.01s
- **`<audio>` 摆放**：必须放在 `<body>` 顶层、和 `<video>` / `<img>` 同级，**不能**放进 `<div data-composition-id="...">` 内
