...
[Phase 2 · 素材盘点]
    ...
    [2.2 · 音频素材]
        ...
        [主问题]
            先告知能力："TTS 有两条路：
            - **本地 TTS**（Kokoro-82M，免费、无需 API key、多语种；中文整体不如英文自然）
            - **MiniMax 云端 TTS**（speech-2.8-hd，要 API key，中文效果比本地 TTS 自然得多；默认 speed=1.6；实测语速比理论慢约 27%）"
            然后问：
            - 旁白怎么搞？四个选项：
                1. **真人录音**（你或同事录，发我 wav/mp3）
                2. **本地 TTS**（免费路线，Kokoro-82M）
                3. **MiniMax 云端 TTS**（要 API key，详见 `references/external-tts.md`）
                4. **不要旁白**（纯文字 + 音乐）
            - 内容调性是营销 / 教程 / 故事 / 资讯哪种？不同调性匹配的 TTS 声音不同
            - 背景音乐打算用版权音乐（Epidemic Sound / Artlist）还是免版税（YouTube Audio Library）？还是不要？
            - 音效（whoosh、click、ding）打算放吗？

        [追问深化]
            **(a) TTS 路径**
                - 用户说"用 AI 配音"→ 你回："什么语言？预算？需要 key 还是要免费的？我能给你两条路：本地 TTS 免费但中文机械一点；MiniMax 云端 TTS 要 key 但中文更自然。挑一个。"
                - 用户说"用本地 TTS"→ 你回："默认速度 1.0；教程类一般 0.7-0.8，营销类 1.1-1.2。你这支是哪种？"
                - 用户说"用 MiniMax"→ 你回："中文默认 male-qn-qingse / female-shaonv 这类，先小段试一句。要 voice_id 列表吗？速度 1.6 是默认，但实测比理论慢 27%，所以 scene 时长不能按字数估，要先生成后用 ffprobe 测。反推 scene duration：max(audio_dur + 2.0, 6.0)。"
                - 用户说"用 GPT 那种 voice"→ 你回："本地 TTS 不要钱；MiniMax 是另一种云 TTS（不是 OpenAI）。要不要都做一段对比？"
        ...
        [接受标准]
            - 旁白路径确定：真人 / 本地 TTS / MiniMax TTS / 无旁白
            - 选 TTS 时：本地 TTS 默认 1.0 速；MiniMax 必填 voice_id + speed + 走 6 步流水线（见 `references/tts-workflow.md`）
        ...
        [不接受的答案]
            ...
            - "AI 配音就行" → 哪个 TTS？本地免费还是云端付费？中文还是英文？
...
[Phase 4 · 视觉主题选定 + 微调]
    [使用顺序]
        先 4.0 字号硬约束告知 → 后 4.1 主题选择 → 后 4.2/4.3/4.4 微调

    [4.0 · 字号硬约束 · 手机可读性]
        [覆盖意图]
            **除非用户明确说「做横屏大屏展示」，否则 9:16 竖屏视频默认超大字号。**
            1080×1920 画布上 24px 正文在手机上等效 ~9px，长辈根本看不清。
            这不在主题可调范围内——每个主题都得在这个基线之上。

            **硬基线：正文 ≥ 64px / 主标题 ≥ 100px / 标签 ≥ 28px / 正文用深色**

        [主问题]
            "你的视频是手机上看对吧（抖音/小红书/视频号）？那我先告诉你：
            文字我会做到超大——正文至少 64px、标题 100px+，保证长辈在手机上也能看清。
            这是我这边的硬标准，不用问直接做。"

        [不接受的答案]
            用户说「不用，小字就好」→ 不采纳。这是可读性硬约束，不是审美偏好。
