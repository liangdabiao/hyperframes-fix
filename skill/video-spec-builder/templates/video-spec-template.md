...
- [ ] TTS 旁白：[local-TTS / MiniMax / human / none] · [voice_id] · [speed] · 输出 [voice/*.mp3]
- [ ] 字幕时间轴：[Whisper / 手工] · 输出 [subtitles/*.json]
- [ ] BGM：[文件 / 来源] · 输出 [bgm.mp3]
- [ ] 其他生成项（抠像、3D 模型、shader ……）
...
- 旁白 source：[local-TTS / MiniMax / human / none]
- 旁白 voice_id：[如 male-qn-qingse / female-shaonv / 本地 TTS 对应 ID]
- 旁白 speed：[本地 TTS 默认 1.0；MiniMax 推荐 1.6]
- 旁白实测总时长：[用 ffprobe 测，scene 时长按 max(audio_dur + 2.0, 6.0) 反推]
- 旁白：[每段旁白的起止时间、来源文件、音量]
